...
def cal_average_distance(players):
    """
    players = [[-1, -0.42], [-1, 0.42], [1, -0.42], [1, 0.42]]
    Average Distance: 0.8746868140531158
    
    players = [[-0.1, -0.1], [-0.1, 0.1], [0.0, -0.02], [0.0, 0.02]]
    Average Distance: 0.1336635923473876
    
    players = [[0.5, 0.21], [-0.5, 0.21], [0.5, -0.21], [0.5, -0.21]]
    Average Distance: 0.5242111685732294
    """
    n = len(players)
    total_distance = 0
    for i in range(1, n):
        for j in range(i + 1, n):
            total_distance += np.tanh(np.linalg.norm(np.array(players[i]) - np.array(players[j])))
...
class MyFeatureEncoder_nvn:
    ...
    def encode(self, obs):
        ...
        average_distance = cal_average_distance(obs["left_team"])
...
class CustomizedWrapper(object):
    def __init__(self):
        self.observation_length = 162

    @staticmethod
    def convert_to_vector(obs, fixed_positions=True):
        def do_flatten(obj):
        ...
        # If there were less than 11vs11 players
        # we backfill missing values with
        # -1.
        # 88 = 11 (players) * 2 (teams) * 2
        # (positions & directions) * 2 (x & y)
        if len(o) < 88:
            o.extend([-1] * (88 - len(o)))
        o.extend(do_flatten(obs['left_team_tired_factor']))
        o.extend(do_flatten(obs['left_team_yellow_card']))
        o.extend(do_flatten(obs['right_team_tired_factor']))
        o.extend(do_flatten(obs['right_team_yellow_card']))
        # ball position
        o.extend(obs['ball'])
        # ball direction
        ...
        o.extend(do_flatten(obs['sticky_actions']))
        active = [0] * 11
        if obs['active'] != -1:
            active[obs['active']] = 1
        o.extend(active)
...
'''
    ...
    def process_state(self, state):
        vec = CustomizedWrapper.convert_to_vector(state)
        return th.from_numpy(vec.astype(np.float32)).unsqueeze(0)

    def process_obs(self, obs):
        if self.obs_type == 1:
            return [process_state(obs)]
...
feature_encoder = MyFeatureEncoder_nvn(5)
